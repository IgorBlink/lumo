...
struct ASTHashResult {
    int nodeCount;
    std::string structureHash; // SHA-256 of DFS node-type sequence
};

class ASTHasher : public ASTVisitor {
public:
    ASTHashResult compute(const ProgramNode& program);
...
private:
    int nodeCount = 0;
    std::string signature; // concatenated node types in DFS order
    void record(const std::string& nodeType);
};
